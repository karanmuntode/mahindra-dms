...
def upload():
    try:
        ...
        if not part_no or not unique_id:
            return jsonify({"msg": "Part No and Unique ID are required"}), 400

        if not doc_type or doc_type not in DOCUMENT_TYPES:
            return jsonify({"msg": f"Invalid document type. Choose from: {', '.join(DOCUMENT_TYPES)}"}), 400

        # Check duplicate
        existing = Document.query.filter_by(
            part_no=part_no, unique_id=unique_id,
            location=user.location, doc_type=doc_type
        ).first()
        if existing:
        ...
        logging.info(f"{user.username} uploaded: {unique_name} [{doc_type}]")

        return jsonify({"msg": "Document uploaded successfully", "doc": doc.to_dict()}), 201
...
def autocomplete():
    try:
        ...
        q = Document.query.filter_by(location=user.location, doc_type=doc_type)

        # Search in both part_no and unique_id
        ...
        seen = set()
        for d in docs:
            if d.part_no not in seen:
                suggestions.append({"label": f"{d.part_no} — {d.unique_id}", "part_no": d.part_no, "unique_id": d.unique_id})
                seen.add(d.part_no)
...
def search():
    try:
        ...
        if not doc_type:
            return jsonify({"msg": "Please select a document type first"}), 400

        query = Document.query.filter_by(location=user.location, doc_type=doc_type)

        if part_no:
            query = query.filter(Document.part_no.ilike(f"%{part_no}%"))

        if unique_id:
            query = query.filter(Document.unique_id.ilike(f"%{unique_id}%"))
        ...
        return jsonify({"msg": "Search failed", "error": str(e)}), 500
...
def list_all():
    try:
        ...
        return jsonify({"msg": "Error fetching documents", "error": str(e)}), 500
...
def delete(id):
    try:
        ...
        # Admin can delete any, users only their location
